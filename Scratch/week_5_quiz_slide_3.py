...
def fx_code(from_cur, to_cur):
    ...
    """
    from_cur_upper = from_cur.upper().strip()
    to_cur_upper = to_cur.upper().strip()
    tick = str()
    if from_cur_upper == "USD":
        tick = f"{to_cur_upper}=X"
    else:
        tick = f"{from_cur_upper}{to_cur_upper}=X"
    return tick
