...
def time_it(func, parms):
    ...
    return _mk_msg(time_delta.days, time_delta.seconds //3600 ,(time_delta.seconds//60)%60, time_delta.seconds%60)
...
def _test_other():
    import time
    def _my_func(info, secs):
        time.sleep(secs)
        print(info)
    res = time_it(_my_func, {'secs':64, 'info':"Testing Succeeded"})
    print(res)
